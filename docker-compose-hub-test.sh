#!/bin/bash
# Azure MCP Server - Docker Compose Hub Image Test Script
# This script tests the Azure MCP Server using pre-built image from Docker Hub
...
SSE_OUTPUT="/tmp/compose_hub_sse_out_$$.txt"
COMPOSE_PROJECT="azure-mcp-hub"
...
echo -e "${BLUE}=============================================${NC}"
echo -e "${BLUE}Azure MCP Server - Docker Hub Image Test${NC}"
echo -e "${BLUE}=============================================${NC}"
...
echo -e "\n${BLUE}Step 1: Pulling Docker image from Docker Hub...${NC}"
...
send_mcp_request 1 "initialize" '{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"compose-hub-test","version":"1.0"}}'
sleep 1
...
# Summary
echo -e "\n${BLUE}=============================================${NC}"
echo -e "${GREEN}All Docker Hub Image tests completed!${NC}"
echo -e "${BLUE}=============================================${NC}"
echo -e "Container: ${GREEN}azure-mcp-server${NC}"
echo -e "Image: ${GREEN}kingtest12132/azure-mcp-server-http:local${NC}"
echo -e "Server URL: ${GREEN}http://$HOST:$PORT${NC}"
echo -e "Session ID: ${GREEN}$SESSION_ID${NC}"

# Show container logs
echo -e "\nContainer logs (last 10 lines):"
docker compose -f "$COMPOSE_FILE" -p "$COMPOSE_PROJECT" logs --tail=10
